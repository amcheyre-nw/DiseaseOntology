...
def build_hierarchy(superparent_ui, sourcename="SNOMEDCT_US", filename='disease_classes.csv'):
    ...
    # cache
    pd.DataFrame(treeDF).to_csv(filename, index=False)
    return
...
if __name__ == "__main__":
    ...
    print("change the superparent_ui here to determine the most senior node in ontology. Default 74732009 = 'Mental Disorder'")
    superparent_ui = '74732009' # Mental Disorder

    # Scrape diseases from UMLS, and represent as a hierarchy in "disease_classes.csv"
    build_hierarchy(superparent_ui)

    treeDF = pd.read_csv('disease_classes.csv')

    # first wikipedia pass
    ops = gen_object_properties(treeDF,
                          ...
                          filename='object_properties.csv'
                                )

    ops = pd.read_csv('object_properties.csv')

    # now run through second pass
    ...
    ops2 = pd.read_csv('object_properties.csv')
